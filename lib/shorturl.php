...
function get_short_url_from_pagename($page, $url = TRUE)
{
	...
	if ($page == '' || $page == $defaultpage) {
		...
		$hash = new V32();
		$hashString = $hash->hash($utf8page);
		$hashNumber = hexdec($hashString);
		$hashids = new Hashids('', SHORT_URL_PAGEID_LENGTH, 'abcdefghijklmnopqrstuvwxyz'); // all lowercase
		$shortid = $hashids->encode($hashNumber);
